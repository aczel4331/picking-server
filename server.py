...
try:
    ...
    print(f"[STARTUP] Limpieza no iniciada: {_e_lim}")
...
def _ml_get_all_orders_cuenta(cuenta_id, fecha_desde=None, fecha_hasta=None):
    """
    Trae SOLO los pedidos PENDIENTES DE IMPRIMIR desde ML.
    Filtra directo en la query con shipping.substatus=ready_to_print,
    así no bajamos miles de pedidos ya procesados.
    Rango por defecto: últimos 3 días (suficiente para pendientes).
    """
    tok = _cuentas.get(cuenta_id, {})
    uid = tok.get("user_id")
    if not uid:
        return {}
    # 3 días es suficiente: un pedido pendiente de imprimir no tiene
    # más de 2-3 días de antigüedad en operación normal
    if not fecha_desde:
        fecha_desde = (datetime.now() - timedelta(days=3)).strftime("%Y-%m-%dT00:00:00.000-00:00")
    else:
        fecha_desde = f"{fecha_desde}T00:00:00.000-00:00"
    if not fecha_hasta:
        fecha_hasta = (datetime.now() + timedelta(days=1)).strftime("%Y-%m-%dT23:59:59.000-00:00")
    else:
    ...
    offset  = 0
    limit   = 50
    MAX_PEDIDOS = 400   # tope de seguridad
    while True:
        if len(pedidos) >= MAX_PEDIDOS:
            logger.info(f"[ML] Tope de {MAX_PEDIDOS} pedidos alcanzado")
            break
        r = _ml_get_cuenta("/orders/search", cuenta_id, params={
            "seller": uid, "order.status": "paid",
            # ── FILTRO CLAVE: solo los que se pueden imprimir ──────────────
        ...
        if r.status_code != 200:
            # Si ML rechaza el filtro shipping.substatus, reintentar sin él
            if offset == 0 and r.status_code == 400:
                logger.warning("[ML] Filtro shipping.substatus rechazado — "
                               "reintentando sin filtro (solo 2 días)")
                _fd2 = (datetime.now() - timedelta(days=2)).strftime(
                    "%Y-%m-%dT00:00:00.000-00:00")
                r = _ml_get_cuenta("/orders/search", cuenta_id, params={
                    "seller": uid, "order.status": "paid",
                    "order.date_created.from": _fd2,
...
def _refresh_pedidos_worker_cuenta(cuenta_id, fecha_desde=None, fecha_hasta=None):
    ...
    try:
        ...
        for p in pedidos.values():
            p["_cuenta"] = cuenta_id
        logger.info(f"[REFRESH] {cuenta_id}: {len(pedidos)} pedidos "
                    f"pendientes de imprimir traídos de ML")
        with _lock:
...
def _carga_inicial_pedidos():
    ...
    # Esperar a que Waitress arranque estable.
    # Con el filtro ready_to_print son pocos pedidos, la carga es liviana.
    time.sleep(30)
    ...
    while intentos < MAX_INTENTOS:
        ...
        try:
            ...
            if pend:
                # Con el filtro ready_to_print son ~50 pedidos, se cubren todos
                total_a_consultar = min(len(pend), 200)
                logger.info(f"[CARGA-INICIAL] Consultando shipments de "
                            f"{total_a_consultar} pedidos...")
                for i in range(0, total_a_consultar, 25):
                    _refrescar_estado_pedidos_bg(pend[i:i+25], limite=25)
                    time.sleep(4)
        ...
        time.sleep(30)   # pausa generosa entre intentos
...
@requiere_api_key
def api_tokens_estado():
    """
    Endpoint ULTRALIVIANO — solo lee memoria, no llama a ML.
    Responde en milisegundos. Sirve para diagnosticar si los tokens
    están cargados sin esperar ninguna consulta externa.
    """
    ...
    for cid, t in _cuentas.items():
        ...
        info[cid] = {
            ...
            "expira_en_min": mins,
            "expirado":      (mins is not None and mins <= 0),
        }
    ...
    return jsonify({
        ...
        "archivo_tokens":  os.path.exists(ML_TOKENS_PATH),
        "ts": _dte.datetime.now(
            _dte.timezone(_dte.timedelta(hours=-3))).strftime("%H:%M:%S"),
    })
...
def api_forzar_carga():
    ...
    # Lanzar la carga en BACKGROUND — no bloquear el request HTTP
    def _worker_carga():
        try:
            ...
            if n > 0:
                ...
                if pend:
                    logger.info(f"[FORZAR-CARGA] Consultando "
                                f"{min(len(pend),100)} shipments...")
                    for i in range(0, min(len(pend), 100), 40):
                        _refrescar_estado_pedidos_bg(pend[i:i+40], limite=40)
                        time.sleep(2)
                    logger.info("[FORZAR-CARGA] ✅ Shipments actualizados")
        except Exception as e:
            import traceback
            logger.error(f"[FORZAR-CARGA] Error: {e}\n"
                         f"{traceback.format_exc()[:500]}")

    threading.Thread(target=_worker_carga, daemon=True,
                     name="ForzarCarga").start()

    return jsonify({
        "ok": True,
        "msg": "Carga iniciada en background. "
               "Consultá /api/diagnostico en 1-2 minutos para ver el resultado.",
        "detalle": detalle,
